"""
RUL and uncertainty evaluation for the IGBT accelerated aging experiment.

Usage (example):
    python experiments/run_igbt_rul_eval.py \
        --config config/igbt.yaml \
        --exp-name igbt \
        --n-samples 20 \
...
This script reuses:
  - config/igbt.yaml
  - datasets/igbt_dataset.IGTBDataset
  - models/diffusion (LinearDegradationModel, MBDDegradationImputation, NoisePredictionNet)

It assumes that a diffusion model has already been trained and saved as:
  results/{exp_name}_noise_net.pt
  results/{exp_name}_degradation.pt

RUL definition (simple, relative):
  - Vce (normalized, channel 0) is treated as degradation indicator.
  - For each generated trajectory, we find the first index t where Vce >= threshold
    (default 0.8); the predicted failure step is t_fail.
  - Given a current observation window of length T (here 256), RUL is defined as
        RUL = max(0, T - 1 - t_fail).
  - If no such t is found, RUL = 0 (the trajectory has not yet reached the failure
    threshold within the window).

Ground-truth RUL:
  - The current IGTBDataset does not provide explicit RUL labels. For now, we treat
    RUL estimates as *relative indicators* and only report consistency metrics
    (e.g., spread across multiple samples). Extending the dataset with true RUL
    labels for IGBT is future work.

Uncertainty metrics:
  - For each window we draw n_samples trajectories and compute the empirical
    90% prediction interval (default) of RUL values.
  - PICP = fraction of (pseudo‑)true RUL that fall inside the interval.
  - Mean interval width = average (upper - lower) across test windows.

NOTE:
  - Because true RUL labels for IGBT are not yet defined in the project, the current
    implementation focuses on the *distribution* of RUL samples and interval widths.
  - Once labels are added, the same code can be extended to compute RMSE / PHM Score.
"""
...
def _load_trained_degradation_and_noise_net(
    ...
    """Helper to rebuild degradation model, noise net, and MBD sampler."""
    data_cfg = cfg.get("data", {})
    diff_cfg = cfg.get("diffusion", {})

    seq_length = data_cfg.get("seq_length", 256)

    # Load degradation parameters
    ...
    # Build diffusion sampler (same hyperparameters as training)
    # feature_size will be set later, once we know dataset feature_size
    diffusion_model = MBDDegradationImputation(
        seq_length=seq_length,
        feature_size=1,  # placeholder; will be updated after dataset is loaded
...
def main() -> int:
    ...
    parser.add_argument("--exp-name", type=str, default="igbt", help="Experiment name prefix used when saving checkpoints")
    parser.add_argument("--n-samples", type=int, default=20, help="Number of trajectory samples per test window")
    parser.add_argument("--interval-quantile", type=float, default=0.9, help="Prediction interval quantile (e.g., 0.9 for 90%% PI)")
    parser.add_argument("--threshold", type=float, default=0.8, help="Failure threshold for normalized Vce")
    ...
    print(f"IGBT RUL eval: train={len(train_ds)}, test={len(test_ds)}, "
    ...
    diffusion_model.feature_size = feature_size
    ...
    # Simple DataLoader over test set
    ...
    all_rul_point = []    # e.g., per-window mean RUL over samples

    for batch in tqdm(test_loader, desc="IGBT RUL/UQ evaluation"):
        x, t_pts, m, _ = batch
        x = x.to(device)           # (1, T, F)
        t_pts = t_pts.to(device)   # (1, T)
        m = m.to(device)           # (1, T, F)

        # x is already normalized in IGTBDataset when normalize=True
        T_len = x.shape[1]
        ...
        for _ in range(n_samples):
            with torch.no_grad():
                # Noise-forward step: sample a random timestep and noised input
                B = x.shape[0]
                ...
                # Use the predicted clean trajectory as input to MBD sampler
                # and let MBD refine/impute missing part (here we treat the
                # second half as "future" to be generated, consistent with
                # C-MAPSS RUL protocol).
                mask = torch.ones_like(x0_pred, dtype=torch.bool)
                half = T_len // 2
                mask[:, half:, :] = False

                imputed = diffusion_model.sample(
                    x0_pred.squeeze(0),  # (T,F)
                    t_pts.squeeze(0),
                    mask.squeeze(0),
                    clip_denoised=True,
                )  # (T,F) on device
            ...
            vce_norm = imputed_np[:, 0]  # assume channel 0 is Vce
        ...
        all_rul_point.append(float(rul_samples.mean()))

    # Since we do not yet have ground-truth RUL labels for IGBT in the dataset,
    # we do not compute RMSE / PHM here. Instead, we report UQ-related statistics.
    ...
    # Without true RUL labels we cannot compute PICP; we report only width statistics.
    uq_report = {
        "n_test": int(N_test),
        ...
        "note": "True RUL labels for IGBT are not yet defined in the project; only interval width statistics are reported.",
    }
    ...
    with open(os.path.join(save_dir, f"{args.exp_name}_rul_uq_igbt.json"), "w", encoding="utf-8") as f:
        json.dump(uq_report, f, indent=2, ensure_ascii=False)

    print("IGBT RUL/UQ evaluation finished.")
